...
:param cb_method: Use this to define a method to be called once data has
been found.
:param ratio: The minium share ratio of a torrent file before it can be
accepted into the renaming process (i.e. not part of this list) You may
set this to 0 to perform no ratio checking.
:param path: The working directory of the script, not used here but is
passed to the `cb_method`
...
Several callbacks are defined here and a call to get_session_state()
is made
"""
...
"""
            ...
            client.core.get_torrents_status({'id': result},
                ['name', 'ratio', 'is_finished', 'files', 'save_path']) \
...
Once the list has been compiled the method defined in `cb_method`
is called.
"""
            torrent_filelist = []
                ...
                if float(ratio) > float(status['ratio']) \
                    or status['is_finished'] is False:
                        for f in status['files']:
                                torrent_filelist.append(os.path.join(
                                status['save_path'], f['path']))
                                
            client.disconnect()
            reactor.stop()
 
            # Catch system exits and ignore them :P.
            try:
