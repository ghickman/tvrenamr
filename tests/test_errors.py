...
class TestExceptionsAreRaised(object):
    ...
    def test_output_format_missing_syntax_exception_is_raised_when_one_of_the_output_format_syntax_snippets_is_missing(self):
        ...
        credentials['series'] = title['series']
        credentials['title'] = title['title']
        assert_raises(OutputFormatMissingSyntaxException, self.tv.build_path, series=credentials['series'], season=credentials['season'], episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'], format='test')
