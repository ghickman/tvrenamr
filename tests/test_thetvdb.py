...
class TestTheTvDb(object):
    ...
    def test_searching_the_tv_db_with_an_ambiguous_name_returns_the_correct_show(self):
        assert_equal(self.tv.retrieve_episode_name('the o.c.', '03', '04', 'thetvdb')['show'], 'The O.C.')
    
    def test_searching_the_tv_db_for_an_incorrect_name_returns_a_show_not_found_exception(self):
        assert_raises(ShowNotFoundException, self.tv.retrieve_episode_name, 'west wing', '04', '01', 'thetvdb')
    ...
    def test_searching_the_tv_db_for_a_specific_episode_returns_the_correct_episode(self):
        ...
        title = self.tv.retrieve_episode_name(show=credentials['show'], season=credentials['season'], episode=credentials['episode'], library='thetvdb')
