...
class TestCore(object):
    ...
    def test_passing_in_a_season_number_to_retrieve_episode_name_returns_the_correct_episode_name_from_that_season(self):
        ...
        title = self.tv.retrieve_episode_name(show=credentials['show'], season='2', episode=credentials['episode'])
    ...
    def test_passing_in_a_season_number_renames_a_file_using_that_season_number(self):
        ...
        credentials['title'] = title['title']
        path = self.tv.build_path(show=credentials['show'], season='2', episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'])
    ...
    def test_passing_in_an_episode_number_returns_the_correct_episode_title(self):
        fn = 'chuck.s1e08.blah.HDTV.XViD.avi'
        credentials = self.tv.extract_episode_details_from_file(fn)
        title = self.tv.retrieve_episode_name(show=credentials['show'], season=credentials['season'], episode='9')
        credentials['show'] = title['show']
        credentials['title'] = title['title']
        path = self.tv.build_path(show=credentials['show'], season=credentials['season'], episode='9', title=credentials['title'], extension=credentials['extension'])
        self.tv.rename(fn, path)
    ...
    def test_setting_the_position_of_a_shows_leading_the_to_the_end_of_the_file_name(self):
        ...
        credentials = self.tv.extract_episode_details_from_file(fn)
        title = self.tv.retrieve_episode_name(show=credentials['show'], season=credentials['season'], episode=credentials['episode'])
        credentials['show'] = self.tv.move_leading_the_to_trailing_the(title['show'])
        credentials['title'] = title['title']
        path = self.tv.build_path(show=credentials['show'], season=credentials['season'], episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'])
        self.tv.rename(fn, path)
    ...
    def test_setting_an_episodes_format_as_name_season_episode_title(self):
        ...
        credentials['show'] = title['show']
        credentials['title'] = title['title']
        assert_equals(self.tv.build_path(show=credentials['show'], season=credentials['season'], episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'], format='%n - %s%e - %t'), 'tests/data/working/Chuck - 108 - Chuck Versus the Truth.avi')
    ...
    def test_setting_an_episodes_format_as_season_episode_title_name(self):
        ...
        credentials['show'] = title['show']
        credentials['title'] = title['title']
        assert_equals(self.tv.build_path(show=credentials['show'], season=credentials['season'], episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'], format='%s - %e - %t - %n'), 'tests/data/working/1 - 08 - Chuck Versus the Truth - Chuck.avi')
    ...
    def test_setting_an_episodes_format_as_title_episode_season_name(self):
        ...
        credentials['show'] = title['show']
        credentials['title'] = title['title']
        assert_equals(self.tv.build_path(show=credentials['show'], season=credentials['season'], episode=credentials['episode'], title=credentials['title'], extension=credentials['extension'], format='%t - %e - %s - %n'), 'tests/data/working/Chuck Versus the Truth - 08 - 1 - Chuck.avi')
