...
When adding options it is sometimes easier to use the long version, especially if you need to pass something in with the switch. This is because 
specifying a short-hand version of a switch, say -r, would require you to add the renamed path like so: @-r'/path/to/dir'@. Whereas the long-hand
version, in this instance, tends to look neater - @--renamed=/path/to/dir@.
...
leading The moved to a trailing The. This is not always a desired option if you want some shows to have the position of the leading The moved and some 
not.
